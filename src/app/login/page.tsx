...
import { pageTitleClass, successTextClass } from "@/lib/ui-classes";

type PageProps = {
  searchParams: Promise<{ next?: string; reset?: string; email?: string; verified?: string }>;
};

export default async function LoginPage({ searchParams }: PageProps) {
  const { next, reset, email, verified } = await searchParams;
  ...
  return (
    <div className="min-h-screen">
      ...
      <main id="main-content" tabIndex={-1} className="mx-auto flex min-h-[60vh] max-w-md flex-col justify-center px-4 py-12">
        ...
        {verified === "1" && (
          <p className={`mb-4 text-center ${successTextClass}`}>이메일 인증이 완료되었습니다. 로그인해 주세요.</p>
        )}
        <LoginForm next={next} />
